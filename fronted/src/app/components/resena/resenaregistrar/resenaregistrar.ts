 ...
 form: FormGroup = new FormGroup({});
  ...
  ngOnInit(): void {
    ...
    this.form = this.fb.group({
      ...
      // la fecha no se muestra en el form; se genera automáticamente
    });
  }
  ...
  aceptar(): void {
    ...
    // fecha automática en formato YYYY-MM-DD (compatible con LocalDate)
    // si ya viene seteada en el modelo, se respeta; si no, se genera ahora:
  ...
  private init(): void {
    if (this.edicion) {
      this.rs.listId(this.id).subscribe(data => {
        // mantener la fecha que viene del backend
